...
class CourseTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!Course::exists()) {
            ...
            $course_names = array(
                array('course_title' => 'Photography - Become a Better Photographer',
                    'instructor_id' => $instructor->getKey() ?? 9,
                    'category_id' => Category::first()->getKey() ?? 9,
                    'instruction_level_id' => InstructionLevel::INTRODUCTORY,
                    'duration' => 16,
                    'price' => 0,
            ...
            foreach ($course_names as $course_key => $course_name) {
                ...
                foreach ($section_names as $s_key => $section_name) {
                    $curriculum_sections['course_id'] = $course->getKey();
                    $curriculum_sections['title'] = $section_name;
                    $curriculum_sections['sort_order'] = $s_key;
                    $curriculum_sections['created_at'] = date("Y-m-d H:i:s");
                    $curriculum_sections['updated_at'] = date("Y-m-d H:i:s");
                    ...
                    $curriculum_lectures_quiz['created_at'] = date("Y-m-d H:i:s");
                    $curriculum_lectures_quiz['updated_at'] = date("Y-m-d H:i:s");
        ...
        $user_id = $instructor->user->getKey();
        ...
        if (!$is_course_taken_exist->count()) {
            ...
            $course_taken->course_id = $course->getKey();
            $course_taken->save();
        }
        ...
        if (!$is_transaction_taken_exist->count()) {
            ...
            $transaction->course_id = $course->getKey();
        ...
        if (!$is_credits_taken_exist->count()) {
            ...
            $credits->course_id = $course->getKey();
